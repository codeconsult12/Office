    ...
    public static class SqlCeLib
    {
        ...
        public static void BulkInsert(string TableName, DataTable datatable, bool DeleteBeforeInsert)
        {
            try
            {
                try
                {
                    ...
                    try
                    {
                        sqlCeCommand.set_Connection(SqlCeLib.sqlCeConn);
                        sqlCeCommand.set_Transaction(SqlCeLib.sqlCeTrans);
                        ...
                        SqlCeResultSet sqlCeResultSet = sqlCeCommand.ExecuteResultSet(1);
                        try
                        {
        ...
        public static void CreateDatabase()
        {
            try
            {
                ...
                try
                {
                    sqlCeEngine.set_LocalConnectionString(SqlCeLib.ConnectionString);
        ...
        public static object Execute(string CommandText, SqlCeLib.ExecMode Mode, params SqlCeParameter[] CommandParameter)
        {
            ...
            try
            {
                try
                {
                    ...
                    try
                    {
                        commandText.set_Connection(SqlCeLib.sqlCeConn);
                        commandText.set_Transaction(SqlCeLib.sqlCeTrans);
                        commandText.CommandType = CommandType.Text;
                        commandText.CommandText = CommandText;
                        commandText.CommandTimeout = 0;
                        commandText.get_Parameters().Clear();
                        if ((int)CommandParameter.Length > 0)
                        {
                            commandText.get_Parameters().AddRange(CommandParameter);
                        }
                        switch (Mode)
                        {
